...
class Ellipsoid(object):
    ...
    def __init__(self, a: numbers.Real, b: numbers.Real, F_0: numbers.Real):
        self.a = a
        self.b = b
        self.n = (a - b) / (a + b)
        self.e2 = (a**2 - b**2) / a**2
...
def dms2rad(
    deg: float | Sequence[float],
    min: float | Sequence[float] = 0.0,
    sec: float | Sequence[float] = 0.0,
) -> np.ndarray:
...
def rad2dms(rad: float | Sequence[float], dms: bool = False) -> np.ndarray:
...
def get_easting_northing_from_gps_lat_long(
    ...
    rads: bool (optional)
        If true, specifies input as radians, otherwise degrees are assumed.
    ...
    rho = datum.a * datum.F_0 * (1 - datum.e2) / (1 - datum.e2 * sin(phi) ** 2) ** 1.5
    ...
    M = (
        ...
        * datum.F_0
        * (
            (1 + datum.n + 1.25 * datum.n**2 + 1.25 * datum.n**3) * (phi - datum.phi_0)
    ...
    const_III = nu / 24.0 * sin(phi) * cos(phi) ** 3 * (5 - tan(phi) ** 2 + 9 * eta**2)
    const_IIIA = (
        nu / 720.0 * sin(phi) * cos(phi) ** 5 * (61 - 58 * tan(phi) ** 2 + tan(phi) ** 4)
    )
    const_IV = nu * cos(phi)
...
def get_gps_lat_long_from_easting_northing(
    east: float | Sequence[float],
    north: float | Sequence[float],
    rads: bool = False,
    dms: bool = False,
    dtype: np.dtype = float64,
) -> tuple[np.ndarray, np.ndarray]:
    ...
    rho = datum.a * datum.F_0 * (1 - datum.e2) / (1 - datum.e2 * sin(phi_dash) ** 2) ** 1.5
    eta2 = nu / rho - 1
    ...
    VII = tphi / (2 * rho * nu)
    VIII = tphi / (24 * rho * nu**3) * (5 + 3 * tphi**2 + eta2 - 9 * (tphi**2) * eta2)
    ...
    lam = (
        datum.lam_0 + X * d_east - XI * d_east**3 + XII * d_east**5 - XIIA * d_east**7
    )
...
def WGS84toOSGB36(
    phi: float | Sequence[float], lam: float | Sequence[float], rads: bool = False
) -> tuple[np.ndarray, np.ndarray]:
