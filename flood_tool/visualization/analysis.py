...
def plot_postcode_density(
    postcode_file: os.PathLike = DEFAULT_POSTCODE_FILE,
    coordinate: tuple[Sequence[float], Sequence[float]] = ["easting", "northing"],
    dx=1000,
) -> None:
    ...
    plt.pcolormesh(X, Y, np.where(Z > 0, Z, np.nan).T, norm=matplotlib.colors.LogNorm())
    plt.axis("equal")
    plt.colorbar()
